...
export default function UpdateModal({ trip, onClose, onUpdate }: UpdateModalProps) {
	const [form, setForm] = useState<TripForm>({ Name: trip.Name });
	...
	async function submit() {
		...
		setError(null);
		try {
			const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/trip/${trip.Id}`, {
				method: 'PATCH',
			...
			setError('Nepavyko redaguoti kelionės');
		} finally {
			setSubmitting(false);
		}
	}
		...
		>
			<div className='w-full max-w-md bg-card rounded-2xl border border-border shadow-xl'>
				<div className='flex items-center justify-between px-6 py-4 border-b border-border'>
					<h2 className="font-['Outfit'] font-bold text-lg text-foreground">Redaguoti kelionė</h2>
					<button
						onClick={onClose}
				...
				<div className='flex items-center justify-end gap-3 px-6 py-4 border-t border-border'>
					...
					>
						{submitting ? 'Redaguomaja...' : 'Redaguoti'}
					</button>
				</div>
			</div>
		</div>
