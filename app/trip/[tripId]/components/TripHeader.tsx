import { Calendar, MapPin, Users, Hotel, Edit, UserPlus, Route } from 'lucide-react';
...
export function TripHeader({ title, status, dateRange, placesCount, participants, hasAccommodation }: TripHeaderProps) {
	return (
		<div className='bg-gradient-to-r from-[#1A6B3C] to-[#2D8A55] rounded-xl p-8 text-white shadow-lg shadow-primary/20'>
			{/* Title and Status */}
				...
				<div>
					...
					<span
						className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${
							status === 'active' ? 'bg-accent text-white' : 'bg-white/20 text-white'
						}`}
					>
						{status === 'active' ? 'Vykdoma' : 'Suplanuota'}
					</span>
				</div>
			</div>
